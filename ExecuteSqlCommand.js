...
function ExecuteSqlCommand(SQL_input, DEBUG) {
    ...
    for (x in SQL_input.param) {
        ...
        if (
            SQL_input.param[x].datatype == DataTypeEnum.adDecimal ||
            SQL_input.param[x].datatype == DataTypeEnum.adNumeric
        ) {
    ...
    try {
        ...
        if (rs.fields.count > 0) {
            ...
            if (
                SQL_input.type == CommandTypeEnum.adCmdStoredProc ||
                SQL_input.type == CommandTypeEnum.adCmdText ||
                SQL_input.type == CommandTypeEnum.adCmdTable
            ) {
                ...
                switch (SQL_input.type) {
                    case CommandTypeEnum.adCmdStoredProc:
                        cmd.CommandText =
                            "SELECT name, system_type_name FROM sys.dm_exec_describe_first_result_set_for_object (OBJECT_ID('" +
                            SQL_input.command +
                            "'),NULL);";
                        break;
                    case CommandTypeEnum.adCmdText:
                        cmd.CommandText =
                            ...
                            SQL_input.command.replace(/'/g, "''") +
                            "',NULL,0);";
                        break;
                    case CommandTypeEnum.adCmdTable:
                        cmd.CommandText =
                            "SELECT name, system_type_name FROM sys.dm_exec_describe_first_result_set ('SELECT * FROM " +
                            SQL_input.command +
                            "',NULL,0);";
                        break;
                }
